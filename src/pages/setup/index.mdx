...
In order to start using `reshadow` you only need `Babel` which you most likeyly already use(for transpiling `JSX` syntax or your code for older browsers). You can also delegate styles processing to `PostCSS`. And extract them into separate files with the provided webpack loader. Please read below.
